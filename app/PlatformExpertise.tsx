...
const platforms = [
  ...
  {name:"AWS",slug:"aws",href:"/platforms/aws",logo:"https://upload.wikimedia.org/wikipedia/commons/f/fc/Amazon_Web_Services_2025.svg"},
];
  ...
  return <section className="platforms" id="platforms">
    <div className="shell platformStrip">
      ...
      <div className="platformLogoStrip" aria-label="Primary technology platforms">
        {platforms.map((platform)=><Link className={`platformLogoItem platform-${platform.slug}`} href={platform.href} key={platform.name} aria-label={`${platform.name} consulting`}><img src={platform.logo} alt={platform.name} loading="eager" decoding="async"/></Link>)}
      </div>
